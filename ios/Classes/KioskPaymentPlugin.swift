...
enum ChannelNameEnum {
    ...
    static let kEventGenerateToken = "kEventGenerateToken"
}
...
public class KioskPaymentPlugin: NSObject, FlutterPlugin, BMSSwiperControllerDelegate {
    ...
    private var generateTokenEventSink: FlutterEventSink?
    ...
    func initChannels(registrar: FlutterPluginRegistrar) {
        ...
        let generateTokenChannel = FlutterEventChannel(name: ChannelNameEnum.kEventGenerateToken, binaryMessenger: registrar.messenger())
        generateTokenChannel.setStreamHandler(GenerateTokenStreamHandler(plugin: self))
    }
    ...
    public func swiper(_ swiper: BMSSwiper, didGenerateTokenWith account: BMSAccount?, completion: @escaping (() -> Void)) {
        if let sink = generateTokenEventSink {
            var data: [String: Any] = [:]
            if let account = account {
                data["token"] = account.token
                data["last4"] = account.last4
                data["expiration"] = account.expirationDate?.description
                data["cardType"] = account.accountType
            }
            sink(data)
        }
        completion()
    ...
    class GenerateTokenStreamHandler: NSObject, FlutterStreamHandler {
        weak var plugin: KioskPaymentPlugin?
        init(plugin: KioskPaymentPlugin) { self.plugin = plugin }
        func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
            plugin?.generateTokenEventSink = events
            return nil
        }
        func onCancel(withArguments arguments: Any?) -> FlutterError? {
            plugin?.generateTokenEventSink = nil
            return nil
        }
    }
}
